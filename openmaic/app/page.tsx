...
function HomePage() {
  ...
  // ── TradingLens → OpenMAIC bridge: hydrate the requirement textarea from a
  // base64url outline supplied as `?r=...` by the Edu Lens deep link.
  // The auth bridge (`?at=`) is handled in middleware.ts which sets the
  // `openmaic_access` cookie before this hook runs. Without this, users
  // would have to retype the outline manually.
  useEffect(() => {
    ...
    try {
      ...
      if (!decoded) return;
      setForm((prev) =>
        prev.requirement ? prev : { ...prev, requirement: decoded },
      );
    } catch (err) {
      log.warn('Failed to decode TradingLens-supplied outline:', err);
    }
  }, []);
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex flex-col items-center p-4 pt-16 md:p-8 md:pt-16 overflow-x-hidden">
      ...
      <div className="mt-auto pt-12 pb-4 text-center text-xs text-muted-foreground/40">
        TradingLens Open Source Project
      </div>
    </div>
  );
}
